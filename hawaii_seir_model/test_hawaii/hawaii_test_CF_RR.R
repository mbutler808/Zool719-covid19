...
a <- ggplot(sims_t1, aes(x = day, y = C, group = .id, color = .id=="data")) +
...
b <- ggplot(sims_t2, aes(x = day, y = C, group = .id, color = .id=="data")) +
  geom_line() + guides(color=FALSE)
...
c <- ggplot(sims_t3, aes(x = day, y = C, group = .id, color = .id=="data")) +
  geom_line() + guides(color=FALSE)
...
d <- ggplot(sims_t4, aes(x = day, y = C, group = .id, color = .id=="data")) +
  geom_line() + guides(color=FALSE)
...
library(ggpubr)
ggarrange(a,b,c,d,e)
ggarrange(a,b,c,d,e + remove("x.text"),
                             labels = c("A", "B", "C", "D", "E"),
                             ncol = 3, nrow =2)
